...
class Manager():
    ...
    def start(self):
        ...
        process_ui = Process(target=self.run_ui_process, args=(self.manager_pipe,), name='process_main')
        process_ui.start()
        self.logger.info('process_ui start ok')
        process_service = Process(target=self.run_service_process, name='process_child', args=(self.manager_pipe,))
